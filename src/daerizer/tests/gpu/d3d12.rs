...
fn the_two_byte_orders_agree_once_swapped() {
    ...
    let shot = |format| {
        ...
        r.draw(&mut t, &geometry, &[inst], &params, d3d12::Mode::Grayscale).expect("draw");
        r.read_pixels(&mut t).expect("read").to_vec()
    };
    let rgba = shot(d3d12::Format::Rgba8Unorm);
    let bgra = shot(d3d12::Format::Bgra8Unorm);
...
fn a_second_geometry_can_draw_over_the_first() {
    ...
    r.draw(&mut layered, &left_geo, &[left], &params, d3d12::Mode::Grayscale).expect("first");
    layered.set_clear(None);
    r.draw(&mut layered, &right_geo, &[right], &params, d3d12::Mode::Grayscale).expect("second");
    ...
    r.draw(&mut cleared, &left_geo, &[left], &params, d3d12::Mode::Grayscale).expect("first");
    r.draw(&mut cleared, &right_geo, &[right], &params, d3d12::Mode::Grayscale).expect("second");
...
fn a_renderer_can_adopt_a_device_it_did_not_create() {
    ...
    let (device, queue) = unsafe { owner.handles() };
    let adopted = unsafe { d3d12::Renderer::from_device(device, queue) }.expect("adopts");
...
fn adopting_a_null_device_is_refused_rather_than_crashing() {
    assert!(
        unsafe { d3d12::Renderer::from_device(core::ptr::null_mut(), core::ptr::null_mut()) }
            .is_err(),
...
fn a_borrowed_texture_draws_what_an_owned_one_does() {
    ...
    for format in [d3d12::Format::Rgba8Unorm, d3d12::Format::Bgra8Unorm] {
        let mut owned = r.target_with_format(w, h, format).expect("owned target");
        r.draw(&mut owned, &geometry, &[inst], &params, d3d12::Mode::Grayscale).expect("draw");
        let direct = r.read_pixels(&mut owned).expect("read").to_vec();

        // D3D12 hands back the resource with its current state; only the resource is needed here.
        let (resource, _state) = unsafe { owned.texture() };
        let mut borrowed = unsafe { r.target_from_texture(resource, w, h, format) }
        ...
        r.draw(&mut borrowed, &geometry, &[inst], &params, d3d12::Mode::Grayscale).expect("draw");
        drop(borrowed);
